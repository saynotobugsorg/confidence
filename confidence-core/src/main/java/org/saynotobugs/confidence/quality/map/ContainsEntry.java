...
import org.saynotobugs.confidence.description.bifunction.*;
...
public final class ContainsEntry<K, V> extends QualityComposition<Map<K, V>>
{
    ...
    {
        super(new Has<Map<K, V>, Iterable<Map.Entry<K, V>>>(Map::entrySet,
            new Implied<>(
                new DescribedAs<>(
                    ...
                    new Contains<>(new DescribedAs<>(
                        new TextAndValue<>("", v -> new Value(v.getKey())),
                        new TextAndValue<>("", v -> new Value(v.getKey())),
                        new Just(""),
                        new EntryOf<>(keyQuality, new Anything())))),
