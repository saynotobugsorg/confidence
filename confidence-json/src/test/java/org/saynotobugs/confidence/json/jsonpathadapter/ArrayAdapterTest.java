...
class ArrayAdapterTest
{
    ...
    void testEmptyArray()
    {
        assertThat(new ArrayAdapter(JsonPath.from("[]"), ""),
            new AllOf<>(
    ...
    {
        assertThat(new ArrayAdapter(JsonPath.from("{\"a\": [12, true, []]}"), "get('a')[2]"),
            new AllOf<>(
                new Has<>(JsonArrayAdapter::length, new Is<>(0)),
                new Has<>(adapter -> adapter.elementAt(-1), absent()),
                new Has<>(adapter -> adapter.elementAt(0), absent()),
    ...
    void test()
    {
        assertThat(new ArrayAdapter(JsonPath.from("[\"a\", 1, true, {}, [], null]"), ""),
            new AllOf<>(
                ...
                new Has<>("object", adapter -> adapter.elementAt(-3), present(new Has<>(JsonElementAdapter::asObject, present(new Anything())))),
