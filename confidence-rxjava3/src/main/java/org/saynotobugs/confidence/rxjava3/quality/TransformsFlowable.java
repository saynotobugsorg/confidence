...
public final class TransformsFlowable<Up, Down> extends QualityComposition<Function<? super TestScheduler, ? extends FlowableTransformer<Up, Down>>>
{

    @SafeVarargs
    public TransformsFlowable(TransformerTestStep<Up, Down>... testSteps)
    {
        this(new Seq<>(testSteps));
    }


    public TransformsFlowable(Iterable<? extends TransformerTestStep<Up, Down>> tesSteps)
    {
        super(new TransformsTestSteps<>(tesSteps));
    }


    public TransformsFlowable(
        Function<? super Scheduler, ? extends Publisher<Up>> upstream,
        Quality<Function<? super TestScheduler, ? extends Publisher<? extends Down>>> downStreamQualities)
    {
        super(new Has<>(
            transformerFunction ->
                (TestScheduler scheduler) -> Flowable.fromPublisher(upstream.value(scheduler))
                    .compose(transformerFunction.value(scheduler)),
            downStreamQualities));
    }

    private static class TransformsTestSteps<Up, Down> implements Quality<Function<? super TestScheduler, ? extends FlowableTransformer<Up, Down>>>
    {
