...
public final class TransformsSingle<Up, Down> extends
    QualityComposition<Function<? super TestScheduler, ? extends SingleTransformer<Up, Down>>>
{

    @SafeVarargs
    public TransformsSingle(TransformerTestStep<Up, Down>... testSteps)
    {
        this(new Seq<>(testSteps));
    }


    public TransformsSingle(Iterable<? extends TransformerTestStep<Up, Down>> testSteps)
    {
        super(new TransformsTestSteps<>(testSteps));
    }


    public TransformsSingle(
        Function<? super Scheduler, ? extends Publisher<Up>> upstream,
        Quality<? super Function<? super TestScheduler, ? extends Single<? extends Down>>> downStreamQualities)
    {
        super(new Has<>(
            transformerFunction ->
                (Function<TestScheduler, Single<Down>>)
                    scheduler -> Single.fromPublisher(upstream.value(scheduler))
                        .compose(transformerFunction.value(scheduler)),
            downStreamQualities));
    }

    private static class TransformsTestSteps<Up, Down> implements Quality<Function<? super TestScheduler, ? extends SingleTransformer<Up, Down>>>
    {
